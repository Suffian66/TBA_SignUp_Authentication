...
namespace User.Management.Controllers
{
    ...
    {
        private readonly ITeacherAttendance _teacherAttendanceService;

        public TeacherAttendanceController(ITeacherAttendance teacherAttendanceService)
        {
            _teacherAttendanceService = teacherAttendanceService;
        }

        [HttpGet("[action]")]
        public async Task<ActionResult<IEnumerable<GetTeacherAttendanceDto>>> GetAllAttendances(DateTime? attendanceDate)
        {
            try
            {
                var attendances = await _teacherAttendanceService.GetAllAttendancesAsync(attendanceDate);
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> AddAttendance([FromBody] List<AddTeacherAttendanceDto> attendanceDto)
        {
            ...
            try
            {
                var results = new List<AddTeacherAttendanceDto>();

                foreach (var attendance in attendanceDto)
                {

                    var result = await _teacherAttendanceService.AddAttendanceAsync(attendance);
                    results.Add(result);
                }
